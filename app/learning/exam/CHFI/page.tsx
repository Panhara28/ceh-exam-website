...
export default function LearningChfiPage(){
    ...
    const totalQuestions = CHFI_DUMP_QUESTIONS.length
    const totalPages = Math.ceil(totalQuestions / questionsPerPage)
    const startIndex = (currentPage - 1) * questionsPerPage
    const endIndex = Math.min(startIndex + questionsPerPage, totalQuestions)
    const currentQuestions = CHFI_DUMP_QUESTIONS.slice(startIndex, endIndex)
    ...
    return (
      <div className="container mx-auto py-8 px-4 max-w-4xl">
                  ...
                  {question.options.map((option, index) => (
                    <div
                      ...
                      className={`p-3 rounded-md border ${
                        index === question.correctAnswer ? "bg-green-50 border-green-200" : "bg-white border-gray-200"
                      }`}
                    >
                      <div className="flex items-start">
                        <div className="mr-2 font-medium">{String.fromCharCode(65 + index)}.</div>
                        <div>{option}</div>
                        {index === question.correctAnswer && (
                          <div className="ml-auto text-green-600 text-sm font-medium">Correct Answer</div>
                        )}
                      </div>
                    </div>
                  ))}
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center gap-4 mt-8">
          ...
          <div className="flex items-center gap-1 flex-wrap justify-center">{generatePaginationButtons()}</div>
